...
class SprzedajemyScraper:
    ...
    SLEEP_TIME = 0.3  # time to wait (in seconds) between requests
    MAX_FAILED_REQS = 15  # maximum of failed requests before stopping
    MAX_CONSECUTIVE_ERRORS = 10  # maximum HTTP errs after which to stop
    ...
    def __init__(self, cache_file: str = None, get_new_last_id: bool = False,
        ...
        In:
          ...
          @get_new_last_id: if True, a new ad id is fetched from a main
                            page on Sprzedajemy; otherwise last attemp-
                            ted id from cache forms the starting point.
        ...
        time.sleep(self.SLEEP_TIME)  # initial sleep just in case

        if get_new_last_id or len(self.parsed_ids) == 0:
            self.current_id = get_recent_ad_id()  # fetches by a request
        elif from_last_id is not None:
            self.current_id = from_last_id
        else:
            self.current_id = self.parsed_ids[-1]

        self.parser = AdParser()  # parses all full HTML documents
    ...
    def next_item(self) -> str:
        """
        Returns a single parsed advertisement text. Sprzedajemy auto-
        ...
        while i < self.MAX_FAILED_REQS and not yielded_text:
            ...
            time.sleep(self.SLEEP_TIME)  # sleep after every request
            self.parsed_ids.append(self.current_id)

            html = get_ad_html_by_id(self.current_id)

            if not html:
                consecutive_error_count += 1

                if consecutive_error_count >= self.MAX_CONSECUTIVE_ERRORS:
                    raise Exception("Max. consecutive errors reached")

                continue  # now attempt a new advertisement
        ...
        if not yielded_text:
            raise Exception("Failed to fetch an item after max. attempts")
